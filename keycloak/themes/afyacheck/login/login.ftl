<#-- Copied from keycloak.v2/login/login.ftl (parent theme), unchanged except the
     brandTitle/brandBody args added to the registrationLayout call below for the split-screen
     panel (see template.ftl). -->
<#import "template.ftl" as layout>
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled?? brandTitle="Welcome back" brandBody="Sign in to continue your assessment, or pick up right where you left off."; section>
    <#if section = "header">
        ${msg("loginAccountTitle")}
    <#elseif section = "form">
        <style>
            /* Arrangement/spacing for this page, kept local to login.ftl rather than the
